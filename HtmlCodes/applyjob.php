...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['resume']) && isset($_FILES['cover_letter']) && isset($_POST['profile'])) {
        ...
        $profile = $_POST['profile']; // Profile input
        ...
        if (empty($uploadError)) {
            ...
            $data = $conn->prepare("INSERT INTO application (JobID, JobSeekerID, ResumeFilePath, CoverLetter, Profile, Status, DateApplied) VALUES (?, ?, ?, ?, ?, 'pending', NOW())");
            $data->bind_param("sssss", $jobId, $jobSeekerId, $resumeFilePath, $coverLetterFilePath, $profile);
...
<head>
    ...
    <link rel="stylesheet" href="css/jobdetail.css?v=1.1">
</head>
<body>
    <header>
        <h1>Apply for Job</h1>
